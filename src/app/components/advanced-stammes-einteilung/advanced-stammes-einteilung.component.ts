...
export class AdvancedStammesEinteilungComponent {
  readonly JSON = JSON;
  ...
  readonly activeRoleFilter = signal<string | null>(null);
  ...
  private readonly anmeldungen$ = this.weekSelectedSubject.pipe(
    distinctUntilChanged(),
    switchMap(groupId => this.churchToolsService.getAdvancedTeilnehmer(groupId))
  );
  ...
  filteredParticipants = computed(() => {
    ...
    const roleFilterId = this.activeRoleFilter();
    ...
    let filtered = all.filter(m => {
      ...
      if (roleFilterId !== null) {
        matchesRoleDropdown = this.getMaRolleValue(m) === roleFilterId;
      }

      return matchesQuery && matchesDropdown && matchesRoleDropdown;
    });
  ...
  async loadGroupsServer() {
    ...
    if (!groupId) return;
    try {
      const data = await firstValueFrom(this.churchToolsService.getAdvancedTeilnehmer(groupId));
      this.originalLoadedCount = data.length;
      this.distributeParticipants(data);
    } catch (err) {
  ...
  onRoleFilterChange(event: Event) {
    const val = (event.target as HTMLSelectElement).value;
    this.activeRoleFilter.set(val || null);
  }
