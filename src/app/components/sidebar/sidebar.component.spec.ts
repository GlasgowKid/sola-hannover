...
jest.mock('../../app.routes', () => ({
  routes: [
    { path: 'mock-1', data: { title: 'Mock Route 1' } },
    { path: 'mock-2', data: { title: 'Mock Route 2' } },
...
describe('SidebarComponent', () => {
  ...
  const createComponent = createComponentFactory({
    ...
    providers: [
      ...
      provideRouter([]),
      { provide: APP_BASE_HREF, useValue: '/ccm/mock-extension/' }
    ]
  });
  ...
  it('should correctly map menu items from app.routes.ts', () => {
    ...
    expect(items[0]).toEqual({ path: '/mock-1', title: 'Mock Route 1' });
    expect(items[1]).toEqual({ path: '/mock-2', title: 'Mock Route 2' });
  });
  ...
  it('should render a navigation link for each menu item', () => {
    ...
    expect(navLinks[0].getAttribute('href')).toBe('/ccm/mock-extension/mock-1');
  });
});